...
async def quiz_generate(request: QuizGenerationRequest):
    try:
        ...
        if not content:
            raise HTTPException(status_code=400, detail="PDF 내용을 불러오지 못했습니다.")

        # ✅ 문제 생성 프롬프트
        prompt = f"""
        다음 내용을 바탕으로 객관식 퀴즈 {request.num_questions}개를 생성하세요.
        ...
        try:
            ...
            logger.error(f"JSON 파싱 실패 → 원본 응답 저장: {result_text[:200]}")
            return {
                "status": "error",
                "message": "LLM 응답 JSON 변환 실패",
        ...
        for q in questions:
            formatted_questions.append({
                "question": q.get("question", "문항 누락"),
                "options": q.get("options", ["보기1", "보기2", "보기3", "보기4"]),
                "correct_answer": q.get("correct_answer", "정답 정보 없음"),
                "explanation": q.get("explanation", "설명 없음")
            })

        return {
            "status": "success",
            "num_generated": len(formatted_questions),
